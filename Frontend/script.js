...
async function updateDashboardStats() {
  try {
    ...
    if (cards.length >= 4) {
      ...
      cards[1].querySelector(".card-value").textContent = stats.activeCourses || 0;
      cards[2].querySelector(".card-value").textContent = stats.graduates || 0;
      cards[3].querySelector(".card-value").textContent = `${stats.successRate || 0}%`;
    }
  } catch (error) {
...
async function handleFormSubmit(e) {
  ...
  try {
    ...
    showNotification(error.message || "Error saving student data", "error");
  } finally {
    hideLoading();
  }
}
...
async function handleCourseFormSubmit(e) {
  ...
  try {
    ...
    console.error("Error:", error);
    showNotification(error.message || "Error saving course data", "error");
  } finally {
    hideLoading();
  }
}
...
function renderStudentTables(studentsToRender) {
  ...
  tables.forEach((table) => {
    ...
    if (!studentsToRender || studentsToRender.length === 0) {
      const colSpan = table.closest("table")?.querySelectorAll("th")?.length || 6;
      table.innerHTML = `
        <tr>
    ...
    studentsToRender.forEach((student) => {
      ...
      row.innerHTML = `
        ...
        <td>${formatDate(student.enrollment_date || student.enrollmentDate)}</td>
        <td>
        ...
        <td class="action-buttons">
          <button class="action-btn edit-btn" onclick="editStudent('${student.id || student._id}')">
            <i class="fas fa-edit"></i> Edit
          </button>
          <button class="action-btn delete-btn" onclick="deleteStudent('${student.id || student._id}')">
...
function renderCourseTable(coursesToRender) {
  ...
  coursesToRender.forEach((course) => {
    ...
    row.innerHTML = `
      ...
      <td class="action-buttons">
        <button class="action-btn edit-btn" onclick="editCourse('${course.id || course._id}')">
          <i class="fas fa-edit"></i> Edit
        </button>
        <button class="action-btn delete-btn" onclick="deleteCourse('${course.id || course._id}')">
          <i class="fas fa-trash"></i> Delete
        </button>
      </td>
    `;
